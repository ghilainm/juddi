...
public class SubscriptionCallbackListener implements org.uddi.v3_service.UDDISubscriptionListenerPortType, Runnable {
    ...
    public static synchronized BindingTemplate start(UDDIClient client, String cfg_node_name, String endpoint,
        ...
        if (ep == null || !ep.isPublished()) {
            if (exception != null) {
                throw new RuntimeException("Unable to start endpoint, view previous errors for reason", exception);
            } else {
                throw new RuntimeException("Unable to start endpoint, view previous errors for reason, it is stranged because no exception was raised.");
            }
