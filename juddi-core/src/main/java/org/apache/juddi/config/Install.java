...
public class Install {
    ...
    protected static void install(Configuration config) throws JAXBException, IOException, ConfigurationException {
        ...
        try {
            ...
            TModel rootTModelKeyGen = buildInstallEntity(fileRootTModelKeygen, TModel.class, config);
            String fileRootBusinessEntity = rootPublisherStr + FILE_BUSINESSENTITY;
            org.uddi.api_v3.BusinessEntity rootBusinessEntity = buildInstallEntity(fileRootBusinessEntity, org.uddi.api_v3.BusinessEntity.class, config);
            ...
            org.uddi.repl_v3.ReplicationConfiguration replicationCfg = buildInstallEntity(fileReplicationConfig, org.uddi.repl_v3.ReplicationConfiguration.class, config);
            if (!alreadyInstalled) {
            ...
            for (String publisherStr : juddiPublishers) {
                ...
                TModel tModelKeyGen = buildInstallEntity(fileTModelKeygen, TModel.class, config);
                String fileBusinessEntity = publisherStr + FILE_BUSINESSENTITY;
                org.uddi.api_v3.BusinessEntity businessEntity = buildInstallEntity(fileBusinessEntity, org.uddi.api_v3.BusinessEntity.class, config);
    ...
    protected static String getRootPartition(TModel rootTModelKeyGen) throws JAXBException, IOException {
    ...
    private static <T> T buildInstallEntity(final String fileName, Class<T> entityClazz, Configuration config) throws JAXBException, IOException, ConfigurationException {
        ...
        return JAXB.unmarshal(reader, entityClazz);
    }
    ...
    public static void installSaveTModel(EntityManager em, String fileName, UddiEntityPublisher publisher, String nodeId, Configuration config)
        ...
        SaveTModel apiSaveTModel = buildInstallEntity(fileName, SaveTModel.class, config);
    ...
    public static UddiEntityPublisher installPublisher(EntityManager em, String fileName, Configuration config)
        ...
        org.apache.juddi.api_v3.Publisher apiPub = buildInstallEntity(fileName, org.apache.juddi.api_v3.Publisher.class, config);
        if (apiPub == null) {
            return null;
        }
    ...
    private static void installReplicationConfiguration(EntityManager em, org.uddi.repl_v3.ReplicationConfiguration replicationCfg, Configuration config, String thisnode) throws DispositionReportFaultMessage, ConfigurationException {
        ...
        removeOldReplicationConfiguration(em);
        em.persist(model);
    }

    private static void removeOldReplicationConfiguration(EntityManager em) {
        List<ReplicationConfiguration> oldReplicationConfigurations = (List<ReplicationConfiguration>) em.createQuery("select c FROM ReplicationConfiguration c order by c.serialNumber desc").getResultList();
        for (ReplicationConfiguration replicationConfiguration : oldReplicationConfigurations) {
            em.remove(replicationConfiguration);
        }
    }
}