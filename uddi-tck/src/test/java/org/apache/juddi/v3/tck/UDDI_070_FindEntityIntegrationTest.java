...
public class UDDI_070_FindEntityIntegrationTest {
    ...
    /*
    *  SELECT DISTINCT t0.entity_key
  FROM j3_business_entity t8,
    j3_uddi_entity t7,
    j3_business_service t6,
    j3_uddi_entity t5,
    j3_binding_template t4,
    j3_uddi_entity t3,
    j3_tmodel_instance_info t2,
    j3_binding_template t1,
    j3_uddi_entity t0
  WHERE (((((t0.entity_key = t3.entity_key)
  AND ((((SELECT COUNT(t9.ID)
    FROM j3_signature t9
    WHERE (t9.binding_template_key = t0.entity_key)) > 0)
  OR ((SELECT COUNT(t10.ID)
    FROM j3_uddi_entity t11,
      j3_signature t10,
      j3_business_service t12
    WHERE ((t10.business_service_key = t11.entity_key)
    AND ((t11.entity_key             = t1.service_key)
    AND ((t12.entity_key             = t11.entity_key)
    AND (t11.DTYPE                   = 'BusinessService' ))))) > 0 ))
  OR ((SELECT COUNT(t13.ID)
    FROM j3_signature t13,
      j3_business_service t17,
      j3_uddi_entity t16,
      j3_business_entity t15,
      j3_uddi_entity t14
    WHERE ((t13.business_key = t14.entity_key)
    AND (((t16.entity_key    = t1.service_key)
    AND ((t17.entity_key     = t16.entity_key)
    AND (t16.DTYPE           = 'BusinessService' )))
    AND ((t14.entity_key     = t17.business_key)
    AND ((t15.entity_key     = t14.entity_key)
    AND (t14.DTYPE           = 'BusinessEntity' )))))) > 0 )))
  AND (t2.tmodel_key         = 'uddi:uddi.org:protocol:serverauthenticatedssl3' ))
  AND ((t1.entity_key        = t0.entity_key)
  AND (t0.DTYPE              ='BindingTemplate' )))
  AND ((t3.entity_key        = t2.entity_key)
  AND ((t4.entity_key        = t3.entity_key)
  AND (t3.DTYPE              = 'BindingTemplate' ))))
     */
    @Ignore(value = "SQL query generated by this test never ends.")
    public void findSignedEntities() throws InterruptedException {
        Assume.assumeTrue(TckPublisher.isEnabled());
        try {
